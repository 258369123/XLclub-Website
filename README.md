...
荣誉页面保留表格样式，适合展示竞赛成绩、项目记录、活动、复盘成果等结构化内容。`category` 用于页面筛选，目前支持：

```ts
"比赛" | "项目"
```

页面上的“其他”不是一个必须填写的数据分类。凡是 `category` 没有匹配到 `比赛` 或 `项目` 的记录，例如 `category: "沙龙"`、`category: "复盘"`，都会自动归入“其他”筛选。
